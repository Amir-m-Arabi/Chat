...
export async function deleteContact(req: express.Request, res: express.Response): Promise<any> {
  const error = validationResult(req);
  if (!error.isEmpty()) {
    return res.status(400).json({ error: error.array() });
  }
  ...
  try {
    ...
    const theirMessages = messages.filter((msg: any) => msg.senderId !== userId);
...
export async function deleteChat(req:express.Request , res:express.Response):Promise<any>{
  const error = validationResult(req)
  if (!error.isEmpty()){
    return res.status(400).json({error : error.array()})
  }
...
export async function getAllChatInContact(req:express.Request , res:express.Response):Promise<any>{
  const error = validationResult(req)
  if (!error.isEmpty()){
    return res.status(400).json({error : error.array()})
  }
  ...
  try{
    ...
    return res.status(200).json({messages:"" , myMessage:myMessage , theirMessage:theirMessage})
